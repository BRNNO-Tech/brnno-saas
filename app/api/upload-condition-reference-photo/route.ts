...
export async function POST(request: NextRequest) {
  try {
    ...
    const safeTierId = tierId.replace(/[^a-zA-Z0-9_-]/g, '_') || 'tier'
    const fileName = `${Date.now()}-${Math.random().toString(36).slice(2, 10)}.${fileExt}`
    const filePath = `${business.id}/${safeTierId}/${fileName}`
    ...
    if (uploadError) {
      console.error('Condition reference photo upload error:', uploadError)
      return NextResponse.json(
    ...
    console.error('Error uploading condition reference photo:', error)
